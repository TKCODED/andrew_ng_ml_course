...
class network:
    def __init__(self, config, inputs, labels, lamda=0):
        ...
        print([layer.prevNOunits for layer in self.layers])
        self.labels = labels
    ...
    def forward(self, input):  # Takes in a datapoint at a time
        ...
        for j in range(1, len(self.config) - 1):
            ...
            self.layers[j].z = [np.dot(self.layers[j].weights[k, :], self.layers[j - 1].activation) for k in
                                range(len(self.layers[j].weights))]
    ...
    def backward(self, input):  # Takes in a datapoint at a time
        ...
        for j in range(len(self.layers) - 1):
            ...
            print("SigmoidGradient:", np.array(self.layers[-2 - j].z))
            print(self.layers[-2 - j].weights[:, 1:].shape)
            print(d[-1].shape)
    ...
    class layer:
        def __init__(self, units, prevNOunits):
            ...
            self.weights = np.array(
                [[random.randint(0, 30) for _ in range(self.prevNOunits + 1)] for _ in range(self.units)])
            self.activation = []
            self.z = []
...
n = network([2, 3, 5], [[3, 5, 3], [4, 3, 7]], [5, 1])
